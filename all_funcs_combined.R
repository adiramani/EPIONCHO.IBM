...

{
...

  {
    ...
    if(calc_ov16) {
      any_juvy_worms <- rowSums(all.mats.temp[, worms.start:num.cols])
      indv_antibody_response <- all.mats.temp[,91]

      Ov16_Seropositive <- ifelse(any_juvy_worms > 0 & indv_antibody_response == 1, 1, 0)
...

  {
    ...
    if(calc_ov16) {
      ...
      new_inf_Ov16 <- which(any_juvy_worms > 0 & Ov16_Seropositive == 0 & indv_antibody_response == 1)
